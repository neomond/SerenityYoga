...
const likedItemsSlice = createSlice({
  ...
  reducers: {
    addItem: (state, action) => {
      ...
      if (!state.likedItems.includes(newItem)) {
        state.likedItems.push(newItem);
      }
    },
    removeItem: (state, action) => {
      const itemToRemove = action.payload;
      state.likedItems = state.likedItems.filter(item => item !== itemToRemove);
    },
    setLikedItems: (state, action) => {
      state.likedItems = action.payload;
    },
...
export const {addItem, removeItem, setLikedItems} = likedItemsSlice.actions;
export default likedItemsSlice.reducer;

// Selectors
export const getLikedItems = (state: RootState) => state.likedItems.likedItems;
